...
public abstract class CharacterizationBase {
    ...
    protected MutableMeasure<Voltage> dynamicVoltage = MutableMeasure.mutable(Units.Volts.of(5.5));
    ...
    protected MutableMeasure<Velocity<Angle>> angularVelocity = MutableMeasure.mutable(Units.RadiansPerSecond.of(0));
    ...
    protected MutableMeasure<Velocity<Velocity<Angle>>> angularAccel = MutableMeasure
            .mutable(Units.RadiansPerSecond.per(Units.Second).of(0));
    ...
    public CharacterizationBase(MechanismType type, Subsystem subsystem) {
        ...
        this.routineConfig = new Config(quasistaticRamp, dynamicVoltage, Units.Seconds.of(20));
    ...
    private Consumer<SysIdRoutineLog> logRotational() {
        return (log) -> {
                log.motor("motor")
                        ...
                        .angularPosition(
                                mechanismAngle.mut_replace(Units.Radians.of(getMechanismPosition())))
                        .angularVelocity(angularVelocity
                                .mut_replace(Units.RadiansPerSecond.of(getMechanismVelocity())))
                        .angularAcceleration(angularAccel.mut_replace(Units.RadiansPerSecond.per(Units.Second)
     ...
     * be in radians. If type is Linear, this should be in meters.
     */
    public abstract double getMechanismPosition();

    /**
     * @return the velocity of the mechanism. If type is set to Rotational, this should
     * be in radians per second. If type is Linear, this should be in meters per second.
     */
     ...
     * be in radians per second squared. If type is Linear, this should be in meters per
     * second squared.
     */
